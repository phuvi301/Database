...
BEGIN
    ...
    LEFT JOIN 
        PAYMENT P ON B.PaymentID = P.Payment_ID 
    WHERE
        (@Keyword IS NULL OR ISNULL(U.FirstName, '') + ' ' + ISNULL(U.LastName, '') LIKE '%' + @Keyword + '%')
    GROUP BY
...
BEGIN
    ...
    IF @MovieID IS NULL
        RETURN N'Loi: MovieID khong duoc NULL';
    ...
    BEGIN
        ...
        WHERE I.Showtime_ID = @ShowtimeID AND I.SeatStatus = 'Sold';
        ...
        SET @Result = @Result + 
            ...
            N' (' + FORMAT(@StartTime, 'dd/MM/yyyy HH:mm:ss') + N'): ' + 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/H1_Test1_2.png}
	\caption{Kết quả Test 2}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/H1_Test1_3.png}
	\caption{Kết quả Test 3}
\end{figure}
...
Giao diện \texttt{/admin/users} được thiết kế để đáp ứng các yêu cầu:
\begin{itemize}
...
Giao diện \texttt{/admin/revenue} được thiết kế theo phong cách tương tự hai màn hình quản lý người dùng, nhưng tập trung hoàn toàn vào việc trực quan hóa doanh thu phim.
Màn hình này giúp quản trị viên kiểm tra trực tiếp kết quả hàm \texttt{fn\_GetMovieRevenue}.
...
\item \textbf{Bảng danh sách phim (Movie Revenue Table):} Khu vực trung tâm của màn hình là một bảng dữ liệu hiển thị từng bộ phim, bao gồm các cột cơ bản như:
      ...
      \end{itemize}
      Bảng được đặt trong một khung cuộn để có thể hiển thị danh sách tất cả các phim. Khi người dùng bấm vào tên phim, hệ thống sẽ nạp dữ liệu chi tiết xuống khu vực bên dưới.
...
\item Sau khi nhận phản hồi từ Backend, phần thân panel hiển thị nội dung là kết quả chạy hàm \texttt{fn\_GetMovieRevenue} (bao gồm tiêu đề phim, từng suất chiếu, tổng doanh thu và tổng số suất chiếu).
\end{itemize}
